...
static int check_lib_hmac(struct hmac_entry *entry, const char *path)
{
	...
	if (gnutls_memcmp(entry->hmac, hmac, HMAC_SIZE)) {
		...
		FIPSLOG_FAILED("POST", "lib-mac", "Calculated MAC using %s for %s does not match", gnutls_mac_get_name(HMAC_ALGO), path);
		gnutls_memset(hmac, 0, HMAC_SIZE);
		return gnutls_assert_val(GNUTLS_E_PARSING_ERROR);
	}
	_gnutls_debug_log("Successfully verified MAC for %s\n", path);
	FIPSLOG_SUCCESS("POST", "lib-mac", "Calculated MAC using %s for %s matches", gnutls_mac_get_name(HMAC_ALGO), path);
...
static int check_binary_integrity(void)
{
	...
	if (ret < 0) {
		FIPSLOG_FAILED("POST", "lib-mac-gnutls", "%s", "test ended - library integrity using SHA256 mac");
		return ret;
	}
	FIPSLOG_SUCCESS("POST", "lib-mac-gnutls", "%s", "test ended - library integrity using SHA256 mac");

	FIPSLOG_SUCCESS("POST", "lib-mac-nettle", "%s", "test started - library integrity using SHA256 mac");
	ret = check_lib_hmac(&hmac.nettle, paths.nettle);
	if (ret < 0) {
		FIPSLOG_FAILED("POST", "lib-mac-nettle", "%s", "test ended - library integrity using SHA256 mac");
		return ret;
	}
	FIPSLOG_SUCCESS("POST", "lib-mac-nettle", "%s", "test ended - library integrity using SHA256 mac");

	FIPSLOG_SUCCESS("POST", "lib-mac-hogweed", "%s", "test started - library integrity using SHA256 mac");
	ret = check_lib_hmac(&hmac.hogweed, paths.hogweed);
	if (ret < 0) {
		FIPSLOG_FAILED("POST", "lib-mac-hogweed", "%s", "test ended - library integrity using SHA256 mac");
		return ret;
	}
	FIPSLOG_SUCCESS("POST", "lib-mac-hogweed", "%s", "test ended - library integrity using SHA256 mac");

	FIPSLOG_SUCCESS("POST", "lib-mac-gmp", "%s", "test started - library integrity using SHA256 mac");
	ret = check_lib_hmac(&hmac.gmp, paths.gmp);
	if (ret < 0) {
		FIPSLOG_FAILED("POST", "lib-mac-gmp", "%s", "test ended - library integrity using SHA256 mac");
		return ret;
	}
	FIPSLOG_SUCCESS("POST", "lib-mac-gmp", "%s", "test ended - library integrity using SHA256 mac");
