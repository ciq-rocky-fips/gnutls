		    ...
		    unsigned bits, gnutls_sign_algorithm_t sigalgo)
{
	...
	if (fips_request_failure(gnutls_pk_get_name(pk), "sign")) {
		...
		ret = gnutls_privkey_sign_data2(key, sigalgo, 0, &signed_data_corrupted, &sig);
	} else {
	...
	if (fips_request_failure(gnutls_pk_get_name(pk), "verify")) {
		...
		ret = gnutls_pubkey_verify_data2(pub, sigalgo, vflags,
			  ...
			  gnutls_privkey_flags_t flags)
{
	...
	if (fips_request_failure(gnutls_pk_get_name(pk), "verify_known_sig")) {
		...
		sig_corrupted.data = (void *)fail_tmp;
		sig_corrupted.size = sig.size;
		ret = gnutls_pubkey_verify_data2(pub, gnutls_pk_to_sign(pk, dig), vflags,
					&signed_data, &sig_corrupted);
	} else {
		ret = gnutls_pubkey_verify_data2(pub, gnutls_pk_to_sign(pk, dig), vflags,
				       &signed_data, &sig);
