...
void doit(void)
{
	...
	FIPS_PUSH_CONTEXT();
	ret = gnutls_pbkdf2(GNUTLS_MAC_SHA256, &pbkdfkey, &iv, 1000, &pbkdf2,
			    sizeof(pbkdf2));
	if (ret < 0) {
	...
	FIPS_PUSH_CONTEXT();
	ret = gnutls_pbkdf2(GNUTLS_MAC_SHA256, &pbkdfkey, &iv, 999, &pbkdf2,
			    sizeof(pbkdf2));
	if (ret < 0) {
		fail("gnutls_pbkdf2 failed\n");
	}
	...
	FIPS_PUSH_CONTEXT();
	ret = gnutls_pbkdf2(GNUTLS_MAC_SHA256, &pbkdfkey, &iv, 1000, &pbkdf2, 13);
	if (ret < 0) {
		fail("gnutls_pbkdf2 failed\n");
	}
	FIPS_POP_CONTEXT(NOT_APPROVED);
